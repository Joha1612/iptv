...
def get_m3u8_url(channel_url, referer):
    ...
    try:
        ...
        parts_dict = {
            match[0]: match[1]
            for match in const_declarations
        }
...
def get_channels(referer):
    ...
    try:
        ...
        for ch in online_channels:
            ...
            # INDIA FILTER (country-based)
            if "india" in country:
                india.append(ch)

            # BANGLADESH FILTER (country-based)
...
sports_channels, india_channels, bangladesh_channels = get_channels(
    referer_url
)
